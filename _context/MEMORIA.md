...
- [x] Lenis smooth scroll (ver sesión 2026-06-25 — cierra el roadmap 7.3→9/10)
...
## SESIÓN 2026-06-25 (continuación — Lenis smooth scroll, cierra el roadmap 9/10)

Commit `d501777`. Spec: `docs/superpowers/specs/2026-06-25-lenis-smooth-scroll-design.md` (reemplaza la parte de Lenis del spec viejo `2026-06-12`, cuya parte de partículas-reactivas-a-Lenis ya había quedado descartada antes).

- `app/components/LenisProvider.tsx` — Context global, una sola instancia de Lenis montada en `app/layout.tsx` envolviendo `{children}`. Integrado con `gsap.ticker` + `lenis.on('scroll', ScrollTrigger.update)` (patrón oficial Lenis+GSAP).
- `syncTouch: false` (default) — scroll táctil en mobile casi nativo, Lenis solo agrega inercia al wheel/trackpad de desktop. Si `prefers-reduced-motion: reduce`, Lenis no se instancia en absoluto (scroll 100% nativo).
- **Fix del riesgo que se venía anotando desde la auditoría:** `ServiciosSection.tsx` bloqueaba el scroll del modal solo con `document.body.style.overflow = 'hidden'`, que por sí solo NO alcanza con Lenis activo (Lenis puede seguir moviendo el scroll por su propio loop sin depender de ese CSS). Se agregó `lenis?.stop()` / `lenis?.start()` vía `useLenis()` en el mismo `useEffect`, manteniendo el `overflow:hidden` como respaldo para el caso sin Lenis.
- No se tocó ningún `ScrollTrigger` existente (columnas de Servicios, replay del Hero) — siguen funcionando porque Lenis mueve el scroll nativo real, no un wrapper/transform custom.
- Verificado: `npm run build` limpio, `npm run dev` levantó sin errores en consola. **Pendiente: confirmación visual/táctil real de Joan en su celular vía Vercel** — probar especialmente que el scroll de fondo no se mueve con el modal de Servicios abierto.

**Con esto se completa el roadmap 7.3→9/10 acordado en la auditoría del 2026-06-24** (Three.js partículas + replay del Hero con ScrollTrigger + Lenis). Pendiente aparte y futuro (no es parte del roadmap): el SVG nuevo que Joan quiere diseñar para el CTA Final (ver nota arriba — no usar flores ahí).
